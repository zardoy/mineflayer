 ...
 * `options` - (необязательно) либо boolean-сокращение для `assistTakeoff`, либо объект со свойствами:
 * `assistTakeoff` - (необязательно) по умолчанию `false`. Если `true`, временно управляет прыжком и движением, чтобы помочь начать полёт на элитрах.
 * `force` - (необязательно) по умолчанию `false`. Если `true`, запрос не будет отклонён только из-за того, что локальное состояние уже считает бота летящим на элитрах.
