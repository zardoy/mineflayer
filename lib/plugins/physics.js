...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  const PHYSICS_CATCHUP_TICKS = maxCatchupTicks ?? 16
  ...
  function doPhysics () {
    ...
    console.log('ticks processed', curProcessed, (now - prevCall))
    prevCall = now
  }
  // physics should be rewritten to timeAccumulator (and only emit multiple packets if needed)
  bot.resetTimeAccumulator = () => {
    console.log('Dropping physics ticks: ' + Math.floor(timeAccumulator / PHYSICS_TIMESTEP))
    timeAccumulator = 0
  }
  setInterval(() => {
    console.log('ticks processed', catchupTicksTotal)
    catchupTicksTotal = 0
  }, 1000)
