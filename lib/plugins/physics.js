...
const { EntityPhysics, EPhysicsCtx, initSetup } = require('@nxg-org/mineflayer-physics-util')
...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  /** @type {ReturnType<import('prismarine-physics').Physics>} */
  const physics = new Physics(bot.registry, world)

  /** @type {import('@nxg-org/mineflayer-physics-util').EntityPhysics} */
  const physics1 = new EntityPhysics(bot.registry)

  /** @type {import('@nxg-org/mineflayer-physics-util').EPhysicsCtx} */
  let ectx // making this later as bot.entity needs to be instantiated (necessary.)
  ...
  function tickPhysics (now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      ...
      ectx = EPhysicsCtx.FROM_BOT(physics1, bot)
      for (const control in controlState) {
        ectx.state.control.set(control, controlState[control])
      }

      const newState = physics1.simulate(ectx, world)

      // // bot.chat(`hey1 ${newState.pos}, ${ectx.state.vel}, ${ectx.state.onGround}`)
      newState.applyToBot(bot)
