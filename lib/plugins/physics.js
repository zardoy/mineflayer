...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function getPendingElytraFlyPreflightError () {
  ...
  function tryStartPendingElytraFly (isJumpRisingEdge) {
    ...
    const preflightError = getPendingElytraFlyPreflightError()
  ...
  bot.elytraFly = async (options = {}) => {
    const assistTakeoff = typeof options === 'boolean' ? options : options.assistTakeoff === true
