...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  let elytraFlySentPhysicsTick = -1
  ...
  function updatePosition (now) {
    ...
    const localElytraCancellationError = getLocalElytraFlyCancellationError()
    if (localElytraCancellationError != null) {
      logElytraFlightDebug(`local-cancel: ${localElytraCancellationError}`)
      clearPendingElytraFly(new Error(localElytraCancellationError))
    }

    tryStartPendingElytraFly()
  ...
  function getElytraFlyPreflightError () {
    if (bot.entity.fallFlying) {
      ...
      return 'Unable to fly from ground'
  ...
  function tryStartPendingElytraFly () {
    ...
    logElytraFlightDebug('sending-start-elytra-flying')
    bot._client.write('entity_action', {
      entityId: bot.entity.id,
    ...
    elytraFlySentPhysicsTick = physicsTickCounter
    pendingElytraFly = false
  }

  function getLocalElytraFlyCancellationError () {
    if (!speculativeElytraFly || elytraFlyTask.done) return null

    if (bot.entity.onGround) {
      return 'Elytra flight cancelled locally: landed before server confirmation'
    }
    ...
    const localElytraFlying = ectx?.state?.elytraFlying ?? bot.entity.elytraFlying
    const localFallFlying = ectx?.state?.fallFlying ?? bot.entity.fallFlying
    const hasPostSendPhysicsTick = physicsTickCounter > elytraFlySentPhysicsTick
    if (hasPostSendPhysicsTick && !localElytraFlying && !localFallFlying) {
      return 'Elytra flight cancelled locally before server confirmation'
    }

    return null
  }

  function logElytraFlightDebug (label) {
    const localElytraFlying = ectx?.state?.elytraFlying ?? bot.entity.elytraFlying
    const localFallFlying = ectx?.state?.fallFlying ?? bot.entity.fallFlying
    console.log('[physics:elytra]', label, {
      onGround: bot.entity.onGround,
      lastSentOnGround: lastSent.onGround,
      pendingElytraFly,
      speculativeElytraFly,
      taskDone: elytraFlyTask.done,
      entityElytraFlying: bot.entity.elytraFlying,
      entityFallFlying: bot.entity.fallFlying,
      pendingConfirmation: bot.entity._pendingElytraFlightConfirmation,
      localElytraFlying,
      localFallFlying,
      jump: controlState.jump,
      forward: controlState.forward,
  ...
  function clearPendingElytraFly (err) {
    if (err != null) {
      logElytraFlightDebug(`clear-pending-error: ${err.message}`)
    }
    pendingElytraFly = false
    elytraFlySentPhysicsTick = -1
  ...
  bot.elytraFly = async () => {
    ...
    const preflightError = getElytraFlyPreflightError()
    if (preflightError != null) {
      logElytraFlightDebug(`elytraFly-call-preflight-fail: ${preflightError}`)
      throw new Error(preflightError)
    }

    logElytraFlightDebug('elytraFly-called')
  ...
  bot._client.on('position', (packet) => {
    ...
    sendPacketPositionAndLook(pos, newYaw, newPitch, false, false)
  ...
  bot.on('entityElytraState', (entity, elytraFlying) => {
    ...
    logElytraFlightDebug(`entityElytraState:${elytraFlying}`)
    if (elytraFlying) {
