...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function syncVelocityToPhysicsState () {
    if (ectx?.state?.vel != null) {
      ectx.state.vel.set(bot.entity.velocity.x, bot.entity.velocity.y, bot.entity.velocity.z)
    }
  }
  ...
  bot._client.on('position', (packet) => {
    ...
    let newYaw, newPitch
    let isAbsoluteTeleport = false

    if (typeof packet.flags === 'object') {
      isAbsoluteTeleport = !packet.flags.x && !packet.flags.y && !packet.flags.z
      vel.set(
      ...
      isAbsoluteTeleport = (packet.flags & 7) === 0
      vel.set(
    ...
    syncVelocityToPhysicsState()

    if (isAbsoluteTeleport) {
      bot.jumpQueued = false
    }
