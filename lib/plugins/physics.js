...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function doPhysics () {
    ...
    catchupTicks = 0
    // do it every ~50ms (yes, it is less precise, might be a problem when paused actually, but otherwise it can be updated every ~100ms instead, which would cause a lot of noticeable camera movement lags)
    tickPhysics(now)
