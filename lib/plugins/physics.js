...
const { EntityPhysics, EPhysicsCtx, PhysicsSettings, initSetup } = require('@nxg-org/mineflayer-physics-util')
...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  // a hacky fix just for this P.O.C.:

  // make physics.yawSpeed also set PhysicsSettings.yawSpeed
  const orgYawSpeed = PhysicsSettings.yawSpeed
  const orgPitchSpeed = PhysicsSettings.pitchSpeed
  Object.defineProperty(PhysicsSettings, 'yawSpeed', {
    get () {
      return physics.yawSpeed ?? orgYawSpeed
    },
    set (value) {
      physics.yawSpeed = value
    }
  })

  // make physics.pitchSpeed also set PhysicsSettings.pitchSpeed
  Object.defineProperty(PhysicsSettings, 'pitchSpeed', {
    get () {
      return physics.pitchSpeed ?? orgPitchSpeed
    },
    set (value) {
      physics.pitchSpeed = value
    }
  })

  initSetup(bot.registry)

  const staticCtx = EPhysicsCtx.FROM_ENTITY_TYPE(physics, bot.registry.entitiesByName.player)

  physics.gravity = staticCtx.gravity
  ...
  function tickPhysics (now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      const ectx = EPhysicsCtx.FROM_BOT(physics, bot)

      for (const control in controlState) {
        ectx.state.control.set(control, controlState[control])
      }

      // set gravity of state to the physics.gravity
      ectx.gravity = physics.gravity ?? ectx.gravity

      physics.simulate(ectx, world).applyToBot(bot)
  ...
  function updatePosition (now) {
    ...
    const maxDeltaYaw = PHYSICS_TIMESTEP * PhysicsSettings.yawSpeed
    const maxDeltaPitch = PHYSICS_TIMESTEP * PhysicsSettings.pitchSpeed
